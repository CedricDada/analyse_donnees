...
def matrix_of_one(n):
    ...
    for i in range(n):
        list.append(1)
    return np.transpose(np.reshape(np.array(list), (1, -1)))

def moyenne(X,D):
    transpose_X=np.transpose(X)
    Z=transpose_X @ D @ matrix_of_one(X.shape[0])
    M=matrix_of_one(X.shape[0])@np.transpose(Z)
    return M

def reduce(X,D):
    return X-moyenne(X,D)
...
def usual_metrix(X):
    ...
    for i in range(len(inverse_result)):
        inverse_result[i]=math.sqrt(inverse_result[i])
...
def completed_pca(X,M,D,path):
    ...
    plt.show()
def completed_pca_representation_cercle_corelation(X,M,D,path):
    #notre matrice a p colonnes et n lignes
...
completed_pca_representation_cercle_corelation(np.transpose(T),usual_weight(T),usual_metrix(T),"iris.csv")